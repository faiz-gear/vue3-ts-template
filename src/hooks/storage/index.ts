...
export default function useStorage() {
  ...
  const getItem = (key: string) => {
    ...
    const { value, storageTime, expire } = JSON.parse(storage.getItem(actualKey)!) as StorageData // eslint-disable-line
  ...
  const getAllItem = () => {
    ...
    for (let i = 0; i < storage.length; i++) {
      const key = storage.key(i)!.replace(prefix, '') // eslint-disable-line
